...
#Extract the images from the bt-miniguide.xml file.
$baseUrl = "http://startsiden.no/tvguide/entity/image/100x150/";
$data_file = "/home/mno/tvguide/bt/data/bt-miniguide.xml";
open(DAT, $data_file) || die("Could not open file!");
@raw_data = <DAT>;
close(DAT);
$SCPIMAGEDIR = $SCPDIR . "images/";

foreach $line (@raw_data){
	@matches = ($line =~ m/entity_id\=\"([0-9a-zA-Z\-]*?)\"/g);
	foreach $ds (@matches){
		if($ds ne ""){
			#print "-> $ds \n";
			system("wget", "-q", "--directory-prefix=/home/mno/tvguide/bt/images", "$baseUrl$ds");

		       $HOSTNR=0;
        		foreach (@SCPHOSTS){
            			$scp = Net::SCP->new( { "host"=>$SCPHOSTS[$HOSTNR], "user"=>$SCPUSER, timeout => 15 } );
            			$scp->cwd($SCPIMAGEDIR);
            			$scp->put("/home/mno/tvguide/bt/images/".$ds) or die $scp->{errstr};
            			$HOSTNR++;
        		}$scp->quit;
			system("rm /home/mno/tvguide/bt/images/$ds");
