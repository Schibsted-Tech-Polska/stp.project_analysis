...
@SCPHOSTS = ('sapub2.medianorge.no','sapub3.medianorge.no','saweb3.medianorge.no','saweb4.medianorge.no');
my $SCPUSER = 'mno';
...
foreach (@URLS){
    ...
    if($error == 0) {
        open FILE, '>/home/mno/disqus/sa/data/' . $FILENAME;
        foreach my $line (@content) {
            print FILE $line;
        }
        close FILE;
        ...
        foreach (@SCPHOSTS){
            ...
            $scp->cwd($SCPDIR);
            $scp->put("/home/mno/disqus/sa/data/".$FILENAME) or die $scp->{errstr};
            $HOSTNR++;
        }$scp->quit;
    }
    $NR++;
