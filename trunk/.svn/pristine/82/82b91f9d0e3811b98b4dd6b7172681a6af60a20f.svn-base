...
my $filepath = "data/bt0FastChannel/post-processing";
...
foreach $site (keys %sendfiles) {
    ...
    $dir = $sendfiles{"$site"}{"dir"} . "/bt0FastChannel";
    ...
    if ($sftp->put("$filepath/$f","$dir/$f")) {
      ...
      print "Upload bt0arts.del to site $site failed. Will try again.\n";
