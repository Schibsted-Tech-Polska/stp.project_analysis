<%--
 * File           : $Header: //depot/escenic/widget-framework/branches/1.4/widget-core-comments/src/main/webapp/template/widgets/comments/view/helpers/commentForm.jsp#2 $
...
<c:choose>
  <c:when test="${comments.allowAnonymousComments and empty sessionScope.user}">
    <c:set var="commentAction"
           value="${operation=='complaint' ? '/forum/addComplaint' : '/forum/addComment'}"/>
  </c:when>
  <c:otherwise>
    <c:set var="commentAction"
           value="${operation=='complaint' ? '/forum/addAuthenticatedComplaint' : '/forum/addAuthenticatedComment'}"/>
  </c:otherwise>
</c:choose>
  ...
  <div class="content">
    ...
    <html:form action="${commentAction}" method="post"
      ...
      <fieldset>
        ...
        <html:hidden property="errorUrl" value="${article.url}#commentsForm"/>
        ...
        <table>
          <c:choose>
            ...
            <c:otherwise>
              <tr class="byline">
                ...
                <td class="field">
                  <html:text property="field(byline)" styleId="comment-form-byline" styleClass="text-field" maxlength="100"/>
                </td>
              </tr>
              ...
              <tr class="email">
                ...
                <td class="field">
                  <html:text property="email" styleId="comment-form-email" styleClass="text-field" maxlength="50"/>
              ...
              <tr class="title">
                ...
                <td class="field">
                  ...
                  <div class="field-status">
                    ...
                    <div id="${titleFieldErrorId}" class="status-left error"><c:out value="${fn:trim(titleErrorMessage)}" escapeXml="true"/></div>
                    <c:remove var="titleErrorMessage" scope="page" />
                    
                    <div id="comment-form-title-length" class="status-right limiter">
                      <span><c:out value="${comments.commentTitleLength}" escapeXml="true"/></span> <fmt:message key="commnet.form.input.text.restriction.message"/>
                    </div>
                  </div>
          ...
          <tr class="body">
            ...
            <td class="field">
              <html:textarea property="body" styleId="comment-form-body" />
              <span class="compulsory">*</span>

              <div class="field-status">
                <c:set var="bodyErrorMessage"><html:errors property="body"/></c:set>
                <div id="${bodyFieldErrorId}" class="status-left error"><c:out value="${fn:trim(bodyErrorMessage)}" escapeXml="true"/></div>
                <c:remove var="bodyErrorMessage" scope="page" />

                <div id="comment-form-body-length" class="status-right limiter">
                  <span><c:out value="${comments.commentBodyLength}" escapeXml="true"/></span> <fmt:message key="commnet.form.input.text.restriction.message"/>
                </div>
              </div>
          ...
          <%-- we will display captcha only when captcha is turned ON and the user is anonymous --%>
          <c:if test="${comments.captcha and empty sessionScope.user}">
            <tr class="captcha">
              <td class="label">
              ...
              <td class="field">
                ...
                <c:set var="captchaErrorMessage"><html:errors property="captcha"/></c:set>
                <p id="${captchaFieldErrorId}" class="error"><c:out value="${fn:trim(captchaErrorMessage)}" escapeXml="true"/></p>
