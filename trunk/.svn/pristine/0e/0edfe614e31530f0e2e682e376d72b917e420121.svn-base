<%--
 * File           : $Header: //depot/escenic/widget-framework/branches/1.4/widget-core-stories/src/main/webapp/template/widgets/stories/view/default.jsp#3 $
...
<div class="${stories.wrapperStyleClass} ${stories.inpageDnDAreaClass}" <c:if test="${not empty stories.styleId}">id="${stories.styleId}"</c:if>>
  ...
  <c:forEach var="item" begin="${stories.begin}" end="${stories.end}" items="${stories.articles}"
    ...
    <%--<div class="article ${firstArticleStyleClass} ${teaserOption}">--%>
    <div class="article  ${cArticle.articleTypeName} ${firstArticleStyleClass} ${teaserOption} ${inpageDnDSummaryClass}">
      ...
      <div class="media ${cArticle.articleTypeName}">
        ...
        <c:if test="${showImage and stories.imagePosition=='top'}">
      ...
      <div class="text">
        ...
        <c:if test="${not empty intro}">
          ...
          <p class="summary">
            ...
            <span class="${inpageLeadtextClass}"><c:out value="${intro}" escapeXml="true"/></span>
          </p>
        </c:if>

        <c:if test="${not empty stories.readMore}">
          <p class="readMore"><a href="${articleUrl}"><c:out value="${stories.readMore}" escapeXml="true"/></a></p>
        </c:if>
