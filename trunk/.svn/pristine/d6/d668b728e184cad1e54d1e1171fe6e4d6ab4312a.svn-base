...
my $filepath = "data/sa0FastChannel/post-processing";
...
foreach $site (keys %sendfiles) {
    ...
    $dir = $sendfiles{"$site"}{"dir"} . "/sa0FastChannel";
    ...
    if ($sftp->put("$filepath/$f","$dir/$f")) {
      ...
      print "Upload sa0arts.del to site $site failed. Will try again.\n";
