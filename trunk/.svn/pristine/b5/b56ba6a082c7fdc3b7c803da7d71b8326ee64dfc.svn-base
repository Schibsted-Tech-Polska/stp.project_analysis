<%--
 * File           : $Header: //depot/escenic/widget-framework/branches/1.4/widget-core-popularList/src/main/webapp/template/widgets/popularList/controller/mostCommented.jsp#1 $
...
<c:set target="${popularList}" property="showCount" value="${fn:trim(widgetContent.fields.showCountCommented)}" />
  ...
  <article:use articleId="${mostCommentedArticle.objId}">
    ...
    <c:choose>
      <c:when test="${article.articleTypeName == 'picture'}">
        ...
        <c:remove var="commentsCount" scope="request"/>
