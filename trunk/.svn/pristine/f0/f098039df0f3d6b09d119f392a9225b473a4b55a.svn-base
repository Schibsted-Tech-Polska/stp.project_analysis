<%--
 * File           : $Header: //depot/escenic/widget-framework/branches/1.4/widget-community-profile/src/main/webapp/template/widgets/profile/view/summary.jsp#2 $
...
--%>
<%-- the purpose of this page is to render the summary view of the profile widget --%>
...
<div class="${profile.wrapperStyleClass}" <c:if test="${not empty profile.styleId}">id="${profile.styleId}"</c:if>>
  ...
  <c:if test="${not empty profile.fields}">
    <div class="fields">
      <c:forEach var="profileFieldEntry" items="${profile.fields}">
        <dl>
