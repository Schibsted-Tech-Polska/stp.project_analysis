<%--
 * File           : $Header: //depot/escenic/widget-framework/branches/1.4/widget-core-popularList/src/main/webapp/template/widgets/popularList/view/mostRead.jsp#2 $
...
<c:if test="${not empty popularListItems}">
  ...
  <div class="${popularList.wrapperStyleClass}" <c:if test="${not empty popularList.styleId}">id="${popularList.styleId}"</c:if>>
    ...
    <c:choose>
      <c:when test="${popularList.showRelatedPicture=='true'
            or popularList.showLeadtext == 'true' or popularList.showCommentCount == 'true'}">
        <div class="content">
          <c:forEach var="popularListItem" items="${popularListItems}" varStatus="status">
            ...
            <div class="article ${firstStyle} ${lastStyle}" <c:if test="${not empty articleDivWidth}">style="width:${articleDivWidth}px;" </c:if>  >
              ...
              <div class="info title ${styleClassForLeadtext} ${styleClassForCommentCount}">
                <!-- title -->
                <h4>
                  <a href="${popularListItem.url}" class="${popularListItem.inpageTitleClass}"><c:out value="${popularListItem.title}" escapeXml="true"/></a>
                  <!-- comments -->
        ...
        <div class="content">
          <ul>
            <c:forEach var="popularListItem" items="${popularListItems}" varStatus="status">
              ...
              <li class="article ${firstStyle} ${lastStyle}" <c:if test="${not empty articleDivWidth}">style="width:${articleDivWidth}px;" </c:if>  >
                <a href="${popularListItem.url}" class="${popularListItem.inpageTitleClass}">
                    <c:out value="${popularListItem.title}" escapeXml="true"/>
                </a>
                <!-- comments -->
                <c:if test="${popularList.showCount=='true'}">
                  (<c:out value="${popularListItem.count}" escapeXml="true"/>)
