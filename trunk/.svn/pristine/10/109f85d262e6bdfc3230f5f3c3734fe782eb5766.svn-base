<%--
 * File           : $Header: //depot/escenic/widget-framework/branches/1.4/widget-core-list/src/main/webapp/template/widgets/list/view/helpers/picture.jsp#2 $
...
<div id="${slideshow.pictureId}">
  <img src="${picture.content.fields.alternates.value[imageRepresentation].href}"
       ...
       <c:if test="${showPictureInfo}">rel="${pictureInfoDivId}" class="${pictureStyleClass}"</c:if> />
