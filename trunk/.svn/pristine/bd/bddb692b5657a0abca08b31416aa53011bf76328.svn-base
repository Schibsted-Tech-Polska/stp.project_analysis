<%--
 * File           : $Header: //depot/escenic/widget-framework/branches/1.4/widget-core-dateline/src/main/webapp/template/widgets/dateline/controller/default.jsp#1 $
...
<c:set var="todaysDateFormat" value="${fn:trim(widgetContent.fields.todaysDateFormat)}" />
<c:if test="${empty todaysDateFormat}">
  <c:set var="todaysDateFormat" value="EEEE dd MMMM yyyy" />
</c:if>

<c:set var="lastUpdatedDateFormat" value="${fn:trim(widgetContent.fields.lastUpdatedDateFormat)}" />
<c:if test="${empty lastUpdatedDateFormat}">
  <c:set var="lastUpdatedDateFormat" value="EEE, MMM dd, yyyy hh:mm a" />
</c:if>

<c:set var="showDifference" value="${fn:trim(widgetContent.fields.showDifference)}" />
