<%--
 * File           : $Header: //depot/escenic/widget-framework/branches/1.4/widget-mobile-mobileMenu/src/main/webapp/template/widgets/mobileMenu/controller/horizontal.jsp#1 $
...
<%-- the general controller has already set a HashMap named 'mobileMenu' in the requestScope --%>
...
<menu:use id="menuTreeView" treeName="${mobileMenu.treeName}">
  <%-- base is the menu item for current section --%>
  <menu:item id="base"/>

  <c:set target="${mobileMenu}" property="treeView" value="${menuTreeView}"/>
  <c:set target="${mobileMenu}" property="base" value="${base}"/>
  ...
  <view:iterate id="menuItem" name="menuTreeView" depth="2" type="com.escenic.menu.MenuItem">
    ...
    <c:if test="${menuItem.level == 1}">
      ...
      <c:choose>
        <c:when
            test="${baseRelation.base || baseRelation.ancestor || baseRelation.parent}"> <%-- the current menu item is active --%>
  ...
  <c:if test="${not empty topMenuItemsList}">
    ...
    <c:if test="${mobileMenu.displayItems=='all'}">
  ...
  <c:choose>
    <c:when test="${mobileMenu.displayItems=='all'}">
      ...
      <c:forEach var="topMenuItem" items="${topMenuItemsList}" varStatus="menuItemStatus">
        ...
        <c:if test="${not empty base and (mobileMenu.depth+0) > 1 and not empty topMenuItem.menuItem.children}">
    ...
    <c:otherwise><%-- it's active-subitems menu, hence display only the subitems of the active menu item --%>
      ...
      <c:if test="${not empty base and (mobileMenu.depth+0) > 1 and not empty activeMenuItem.children}">
  ...
  <c:set target="${mobileMenu}" property="levels" value="${horizontalLevelMapsList}"/>
  <c:remove var="horizontalLevelMapsList" scope="request"/>
</menu:use>

<c:choose>
  <%--c:when test="${not empty mobileSubMenu.sectionUniqueName}"--%>
  <c:when test="${false}">

    <article:list id="articleList"
                  sectionUniqueName="News"
                  max="5"
                  includeSubSections="false"
                  sort="publishDate"
                  from="${requestScope.articleListDateString}"/>
    <c:set var="articleMenuList" value="${articleList}" scope="request"/>
    <c:set target="${mobileSubMenu}" property="articles" value="${articleList}"/>
  </c:when>
  <c:when test="${not empty mobileSubMenu.subMenuName}">
    <%-- Same thing for the submenu--%>
    <%-- the general controller has already set a HashMap named 'mobileMenu' in the requestScope --%>
    <jsp:useBean id="mobileSubMenu" type="java.util.HashMap" scope="request"/>

    <%-- create a generic view named 'menu' from the given ${menu.treeName} --%>
    <menu:use id="menuTreeView" treeName="${mobileSubMenu.subMenuName}">
      <collection:createList id="topSubMenuItemsList" type="java.util.ArrayList"/>
      <view:iterate id="menuItem" name="menuTreeView" depth="2">
        <c:if test="${menuItem.level == 1}">
          <jsp:useBean id="currentSubMenuItem" class="java.util.HashMap"/>
          <c:set target="${currentSubMenuItem}" property="menuItem" value="${menuItem}"/>
          <c:set target="${currentSubMenuItem}" property="url" value="${menuItem.URL}"/>
          <c:set target="${currentSubMenuItem}" property="text" value="${menuItem.text}"/>

          <collection:add collection="${topSubMenuItemsList}" value="${currentSubMenuItem}"/>
          <c:remove var="currentSubMenuItem" scope="page"/>
        </c:if>
      </view:iterate>
      <c:set target="${mobileSubMenu}" property="items" value="${topSubMenuItemsList}"/>
    </menu:use>
  </c:when>
</c:choose>
