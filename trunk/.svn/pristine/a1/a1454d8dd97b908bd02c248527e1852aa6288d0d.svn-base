<%--
 * File           : $Header: //depot/escenic/widget-framework/branches/1.4/widget-core-menu/src/main/webapp/template/widgets/menu/controller/helpers/verticalAllSubitems.jsp#1 $
...
--%>
<%-- this is the helper controller for the vertical all subitems view of menu widget --%>
...
<jsp:useBean id="parentMenuItemMap" type="java.util.HashMap" scope="request"/>
...
<util:profiler path="/template/widgets/menu/controller/helpers/verticalAllSubitems.jsp">
  <c:set var="parentMenuItem" value="${parentMenuItemMap.menuItem}" />
  <c:set var="currentDepth" value="${parentMenuItemMap.depth+1}" />
  ...
  <view:iterate id="menuItem" name="menuTreeView" depth="${currentDepth+1}" type="com.escenic.menu.MenuItem">
    ...
    <c:if test="${menuItem.level == currentDepth and parentMenuItemRelation.child}">
      ...
      <%--
        call 'verticalAllSubitems.jsp' itself,
      ...
      <c:if test="${(menu.depth+0) > (currentDepth+0) and not empty menuItem.children}">
        <template:call file="verticalAllSubitems.jsp">
          <template:parameter key="parentMenuItemMap" name="currentMenuItemMap" />
        </template:call>
      </c:if>
  ...
  <c:set target="${parentMenuItemMap}" property="submenuItems"  value="${submenuItemMapsList}" />
