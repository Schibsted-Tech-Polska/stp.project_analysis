<%--
 * File           : $Header: //depot/escenic/widget-framework/branches/1.4/widget-community-profile/src/main/webapp/template/widgets/profile/controller/extended.jsp#1 $
...
--%>
<%-- this is the controller for the extended view of the profile widget --%>
...
<article:use articleId="${profileUser.articleId}">
  ...
  <collection:createList id="panelsList" type="java.util.ArrayList"/>

  <c:forEach var="panel" items="${articleType.panels}">
    <jsp:useBean id="panelMap" class="java.util.HashMap" />
    <c:set target="${panelMap}" property="label" value="${panel.label}" />
    <c:set target="${panelMap}" property="description" value="${panel.description}" />
    ...
    <c:forEach var="field" items="${panel.fields}">
      ...
      <c:if test="${fieldType >=1 and fieldType <=5 and fn:toLowerCase(field.name) != 'fullname'}">
    ...
    <c:set target="${panelMap}" property="fields" value="${fieldMap}" />
    <collection:add collection="${panelsList}" value="${panelMap}" />
    <c:remove var="fieldMap" scope="page"/>    
    <c:remove var="panelMap" scope="page" />
  </c:forEach>

  <c:set target="${profile}" property="panels" value="${panelsList}" />
</article:use>
