<%--
 * File           : $Header: //depot/escenic/widget-framework/branches/1.4/widget-framework-core/src/main/webapp/template/framework/tabs/accordions.jsp#2 $
...
<div id="${tempTabUniqueId}" class="accordions">
  <!-- handle requestScope.tabbingEnabled -->
  ...
  <c:forEach var="tabbingGroupItem" items="${tabItemsList}" varStatus="status">
    ...
    <h2 id="${tempTabUniqueId}-${status.count}" <c:if test="${status.first}">class='current'</c:if> <c:if test="${tabbingGroupItemReloadPage == 'true'}">onclick="window.location.href='${reloadPageUrl}';"</c:if> >
        <c:out value="${tabbingGroupItemTitle}" escapeXml="true"/>
    </h2>

    <div class="accordionPane" <c:if test="${status.first}">style='display:block;'</c:if> >
      <c:choose>
        <c:when test="${tabbingGroupItem.type=='tabPaneGroup'}">
    ...
    <c:when test="${changeUrl}">
      <script type="text/javascript">
        ...
        $(document).ready(function() {
          $("#${tempTabUniqueId}").tabs("#${tempTabUniqueId} > div.accordionPane", {
            tabs: 'h2',
            current: 'current',
            effect: 'slide',
            event: '${tempTabEvent}',
