<%--
 * File           : $Header: //depot/escenic/widget-framework/branches/1.4/widget-core-video/src/main/webapp/template/widgets/video/controller/windowsMediaPlayer.jsp#1 $
...
--%>
<%-- this is the controller for the windowsMediaPlayer view of the video widget --%>
...
<c:set target="${video}" property="autoPlay" value="${fn:trim(widgetContent.fields.autoPlayWindowsMediaPlayer)}"/>
<c:set target="${video}" property="repeat" value="${fn:trim(widgetContent.fields.repeatWindowsMediaPlayer)}"/>
...
<c:choose>
  ...
  <c:when test="${requestScope['com.escenic.context'] == 'art' and article.articleTypeName=='simpleVideo' and video.source=='article'}" >
    <collection:createList id="videoContents" type="java.util.ArrayList" toScope="request"/>
