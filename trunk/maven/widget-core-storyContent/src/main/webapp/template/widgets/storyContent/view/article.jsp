<%--
 * File           : $Header: //depot/escenic/widget-framework/branches/1.4/widget-core-storyContent/src/main/webapp/template/widgets/storyContent/view/article.jsp#3 $
      ...
      <c:choose>
        ...
        <c:when test="${currentField eq 'byline'}">
          <p class="${currentField}">
            <c:out value="${storyContent.bylinePrefix}"/>&nbsp;<span class="authorName">
              <c:out value="${storyContent.byline}" escapeXml="true"/></span>
          </p>
        </c:when>
        <c:when test="${currentField eq 'dateline'}">
          <p class="${currentField}">
            ...
            <span class="label"><fmt:message key="article.dateline.updated.label"/></span> ${storyContent.lastModifiedDate}
          </p>
        </c:when>
        ...
        <c:when test="${currentField eq 'tags'}">
          <c:if test="${not empty storyContent.tagList and fn:length(storyContent.tagList) > 0}">
            <div class="${currentField}">
