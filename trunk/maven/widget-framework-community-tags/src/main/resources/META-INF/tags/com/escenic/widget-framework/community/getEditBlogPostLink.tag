<%--
 * File           : $Header: //depot/projects/widget-framework/src/main/webapp/WEB-INF/tags/escenic-framework/getEditStoryLink.tag#1 $
...
--%>
<%--The purpose of this tag is to create an edit story link --%>
...
<profile:present>
  <c:set var="editStorySectionUniqueName" value="saveBlogPost" />
  ...
  <c:if test="${requestScope['com.escenic.context']=='art' and article.articleTypeName eq 'blogPost' }">
  ...
  <c:if test="${not empty artId}">
     <article:use articleId="${artId}">
        <c:if test="${article.articleTypeName eq 'blogPost'}">
          <auth:hasRole role="SECTION OWNER" sectionId="${sectionId}" userId="${currentUser.id}">
            <section:use uniqueName="${editStorySectionUniqueName}">
              <c:set var="editStorySectionUrl" value="${section.url}"/>
            </section:use>

            <c:set var="formId" value="loadStoryForm${articleId}"/>
            <html:form styleId="${formId}" action="/community/loadStoryForm">
              <html:hidden property="articleId" value="${article.id}"/>
              <html:hidden property="homeSectionId" value="${homeSectionId}"/>
              <html:hidden property="successUrl" value="${editStorySectionUrl}"/>
              <html:hidden property="errorUrl" value="${article.url}"/>
            </html:form>

            <c:choose>
              <c:when test="${not empty fn:trim(linkText)}">
                <c:set var="editStoryLinkText" value="${fn:trim(linkText)}" />
              </c:when>
              <c:otherwise>
            ...
            <a href="#" <c:if test="${not empty fn:trim(styleId)}"> id='${fn:trim(styleId)}'</c:if>
               ...
               onclick="$('#${formId}').submit(); return false;"><c:out value="${editStoryLinkText}" escapeXml="true"/></a>
          </auth:hasRole>
        </c:if>
      </article:use>
  </c:if>
