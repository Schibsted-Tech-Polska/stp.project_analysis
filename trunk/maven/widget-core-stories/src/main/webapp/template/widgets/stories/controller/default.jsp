<%--
 * File           : $Header: //depot/escenic/widget-framework/branches/1.4/widget-core-stories/src/main/webapp/template/widgets/stories/controller/default.jsp#1 $
...
<c:set var="begin" value="${fn:trim(widgetContent.fields.beginDefault.value)}"/>
<c:if test="${not empty begin and begin > 0}">
  <c:set var="begin" value="${begin-1}"/>
</c:if>
<c:set var="end" value="${fn:trim(widgetContent.fields.endDefault.value)}"/>
...
<%--<c:set var="imageWidth" value="${fn:trim(widgetContent.fields.imageWidthDefault.value)}" scope="page" />--%>
<%--<c:if test="${stories.imagePosition!='top'}">--%>
  <%--<c:choose>--%>
  	...
  	<%--<c:when test="${imageWidth=='140px'}">--%>
  	  <%--<c:set target="${stories}" property="imageVersion" value="${requestScope.elementwidth < 140 ? 'w80' : 'w140'}" />--%>
  	<%--</c:when>--%>
  	<%--<c:when test="${imageWidth=='220px'}">--%>
  	  <%--<c:set target="${stories}" property="imageVersion" value="${requestScope.elementwidth < 220 ? 'w80' : 'w220'}" />--%>
  	<%--</c:when>--%>
  	<%--<c:when test="${imageWidth=='300px'}">--%>
  	  <%--<c:set target="${stories}" property="imageVersion" value="${requestScope.elementwidth < 300 ? 'w140' : 'w300'}" />--%>
  	<%--</c:when>--%>
  	<%--<c:otherwise>--%>
  	  <%--<c:set target="${stories}" property="imageVersion" value="${requestScope.elementwidth >= 300 ? 'w140' : 'w80'}" />--%>
  	<%--</c:otherwise>--%>
  <%--</c:choose>--%>
<%--</c:if>--%>
