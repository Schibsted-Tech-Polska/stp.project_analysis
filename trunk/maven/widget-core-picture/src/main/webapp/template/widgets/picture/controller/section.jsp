<%--
 * File           : $Header: //depot/escenic/widget-framework/branches/1.4/widget-core-picture/src/main/webapp/template/widgets/picture/controller/section.jsp#1 $
...
--%>
<%-- this is the controller for the default view of the picture widget --%>
...
<c:set target="${picture}" property="groupName" value="${fn:trim(widgetContent.fields.groupName.value)}"/>
<c:set target="${picture}" property="maxPictures" value="${fn:trim(widgetContent.fields.maxPictures.value)}"/>
<c:set target="${picture}" property="showWidgetName" value="${fn:trim(widgetContent.fields.showWidgetName.value)}"/>
<c:set target="${picture}" property="widgetName" value="${fn:trim(element.fields.title.value)}"/>
<c:set target="${picture}" property="includeArticleRelatedPictures" value="${fn:trim(widgetContent.fields.includeArticleRelatedPicturesSection.value)}"/>
...
<c:if test="${not empty picture.groupName}">
  ...
  <wf-core:getPicturesInGroup var="picturesList" group="${targetGroup}" contentType="${picture.contentType}" max="${picture.maxPictures}" includeArticleRelatedPictures="${picture.includeArticleRelatedPictures}"/>
