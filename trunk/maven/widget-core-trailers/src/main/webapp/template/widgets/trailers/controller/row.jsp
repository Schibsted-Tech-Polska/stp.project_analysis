<%--
 * File           : $Header: //depot/escenic/widget-framework/branches/1.4/widget-core-trailers/src/main/webapp/template/widgets/trailers/controller/row.jsp#1 $
...
<c:set target="${trailers}" property="maxTrailers" value="${fn:trim(widgetContent.fields.maxTrailersRow.value)}"/>
<c:set target="${trailers}" property="showSectionName" value="${widgetContent.fields.showSectionNameRow.value}"/>
<c:set target="${trailers}" property="showComments" value="${fn:trim(widgetContent.fields.showCommentCountRow.value)}" />
<c:set target="${trailers}" property="trailerWidthRow" value="${widgetContent.fields.trailerWidthRow.value}"/>
<c:set target="${trailers}" property="imagePositionRow" value="${fn:trim(widgetContent.fields.imagePositionRow.value)}"/>
<c:set target="${trailers}" property="showTitleRow" value="${widgetContent.fields.showTitleRow.value}"/>
<c:set target="${trailers}" property="showIntroRow" value="${widgetContent.fields.showIntroRow.value}"/>
<c:set target="${trailers}" property="maxCharactersRow" value="${fn:trim(widgetContent.fields.maxCharactersRow)}"/>
...
<c:forEach var="articleSummary" items="${requestScope.groupArticleSummaries}" varStatus="loopStatus">
  <c:if test="${loopStatus.count <= trailers.trailerColumnCount}">
    ...
    <jsp:include page="helpers/imageVersionInRowView.jsp"/>
