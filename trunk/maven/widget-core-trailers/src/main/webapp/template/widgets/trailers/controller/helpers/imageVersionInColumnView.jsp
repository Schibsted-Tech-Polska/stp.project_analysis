<%--
 * File           : $Header: //depot/escenic/widget-framework/branches/1.4/widget-core-trailers/src/main/webapp/template/widgets/trailers/controller/helpers/imageVersionInColumnView.jsp#1 $
...
<%--choose an appropriate image version --%>
<c:choose>
  <c:when test="${empty trailers.imageVersion}">
    <c:choose>
      <c:when test="${trailers.trailerWidthColumn eq '140px'}">
        <c:choose>
          ...
          <c:otherwise>
            <c:choose>
              <c:when test="${articleAttributeMap.articleStyleClass eq 'first' and trailers.showIntroColumn eq 'none'}">
                <c:set target="${articleAttributeMap}" property="imageVersion" value="w140"/>
              </c:when>
              <c:when test="${articleAttributeMap.articleStyleClass != 'first' and trailers.showIntroColumn != 'all'}">
                <c:set target="${articleAttributeMap}" property="imageVersion" value="w140"/>
              </c:when>
              <c:otherwise>
      ...
      <c:when test="${trailers.trailerWidthColumn eq '220px'}">
        <c:choose>
          <c:when test="${trailers.imagePositionColumn eq 'top'}">
      ...
      <c:when test="${trailers.trailerWidthColumn eq '300px'}">
        <c:choose>
          <c:when test="${trailers.imagePositionColumn eq 'top'}">
