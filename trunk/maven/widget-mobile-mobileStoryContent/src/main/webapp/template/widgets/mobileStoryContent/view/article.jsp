<%--
 * File           : $Header: //depot/escenic/widget-framework/branches/1.4/widget-mobile-mobileStoryContent/src/main/webapp/template/widgets/mobileStoryContent/view/article.jsp#3 $
...
<%@ taglib uri="http://www.escenic.com/taglib/escenic-community" prefix="community" %>
      ...
      <c:choose>
        ...
        <c:when test="${currentField eq 'byline'}">
          <dxf:p cssClass="${currentField}">
            <c:out value="${mobileStoryContent.bylinePrefix}" escapeXml="true"/>&nbsp;<dxf:span cssClass="authorName"><c:out
              value="${mobileStoryContent.byline}" escapeXml="true"/></dxf:span>
          </dxf:p>
        </c:when>
        <c:when test="${currentField eq 'dateline'}">
          <dxf:p cssClass="${currentField}">
            ...
            <dxf:span cssClass="label"><fmt:message key="article.dateline.updated.label"/></dxf:span> <c:out value="${mobileStoryContent.lastModifiedDate}" escapeXml="true"/>
          </dxf:p>
        </c:when>
        ...
        <c:when test="${currentField eq 'quote'}">
          <c:if test="${not empty mobileStoryContent.quote}">
            <dxf:div cssClass="${currentField}">
              <dxf:div><p><c:out value="${mobileStoryContent.quote}" escapeXml="true"/></p></dxf:div>
            </dxf:div>
          </c:if>
        </c:when>
        ...
        <c:when test="${currentField eq 'tags'}">
          <c:if test="${not empty mobileStoryContent.tagList and fn:length(mobileStoryContent.tagList) > 0}">
            <dxf:div cssClass="${currentField}">
              <jsp:include page="helpers/tags.jsp"/>
            </dxf:div>
          </c:if>
        </c:when>
      </c:choose>
