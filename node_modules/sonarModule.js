...
MavenProject.prototype.execute = function(){
    ...
    executeCommands(this.options, commands);
}
...
function checkTypeOfJavaProject(projectLocation){
	...
	finder.on('file', function (file) {
		...
		if(file.indexOf('pom.xml') != -1){
			...
			//typeOfProject = 'maven';
			typeOfProject = new JavaProject(options);
			typeOfProject.commands[0] = 'mvn clean install';
            typeOfProject.commands[1] = 'mvn sonar:sonar';
			locationsOfBuildFiles.push(file);
			
		}else if(file.indexOf('build.xml') != -1){
			typeOfProject = new JavaProject(options);
			typeOfProject.commands[0] = 'ant build';
            typeOfProject.commands[1] = sonarRunnerCommand;
			locationsOfBuildFiles.push(file);
		}else {
			//typeOfProject = 'gradle';
			typeOfProject = new JavaProject(options);
			typeOfProject.commands[0] = sonarRunnerCommand;
            typeOfProject.commands[1] = 'none';
			locationsOfBuildFiles.push(file);
		}
...
function executeCommands(options,commands,outerCallback){
	...
	/*flow.series([
		function(callback){
		functionToExec(callback)
		},
		function(callback){
		functionToExec2(callback)
		}
	]);*/
