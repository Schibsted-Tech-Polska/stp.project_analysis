...
//--------------------------------------
function JavaProject(options){
	this.options = options;
	this.commands = [];
};
JavaProject.prototype.execute = function(){
	executeCommands(this.options, this.commands);
};
JavaProject.prototype.toString = function(){
...
exports.analyze = function(properties){
	...
	flow.series([
		...
		//,
		//function(callback){
		//	fileModule.deleteFolder(properties.projectLocation);
		//}		
...
function startJavaClient(properties){
	...
	if(javaBuildCommand){
		...
		var options = {
	 	 ...
	 	 executeCommands(options,commands);
...
function generatePropertiesFile(properties ,propertiesToChange, callback) {
	...
	if(language === 'js'){
		...
		//var srcFinder = new sourceFinder.SourceFinderModule(["jquery"],[".js"]);
		//srcFinder.findSrcLocation(projectLocation ,propertiesToChange, saveNewProperties);
		var parameters = {
			'projectLocation':projectLocation,
			'extension':'.js',
			'filesToOmit':jsFilesToOmit.getFiles().concat(properties.filesToOmit.split(coma))
		};
		sourceFinder.findSrcLocation(parameters ,propertiesToChange, saveNewProperties);

    }else if(language === 'php'){
    		var emptyArray =[];
    		
    		var filesTypedByUser = properties.filesToOmit.split(coma);
    		var afterConcat = emptyArray.concat(filesTypedByUser);
    		console.log("after concat : " + afterConcat);

    		var parameters = {
			'projectLocation':projectLocation,
			'extension':'.php',
			'filesToOmit':afterConcat
		};
		sourceFinder.findSrcLocation(parameters ,propertiesToChange, saveNewProperties);
...
function checkTypeOfJavaProject(projectLocation){
	...
	finder.on('file', function (file) {
		...
		if(file.indexOf('pom.xml') != -1){
			...
			//typeOfProject = 'maven';
			var currentFindProject = new JavaProject(options)
...
/*
function executeCommands(options,commands,outerCallback){
	console.log("receive commands : " + commands);
	...
	commands.map(function(currentCommand){
		...
		//var functionToExec = arrayOfExec.pop();
		//var functionToExec2 = arrayOfExec.pop();
